...
for inflowRate in np.arange(u_lowerbound, u_upperbound + u_step, u_step):
        ...
        for v, w in network.edges():
            ...
            if intervalCounter in oldInterval.keys():
                if e not in thinFlow.resettingEdges:
                    if abs(thinFlow.NTFNodeLabelDict[v] - thinFlow.NTFNodeLabelDict[w]) < TOL < abs(
                            oldNodeLabel[v] - oldNodeLabel[w]):
                        clusterJoin.append(
                            [(startTime, inflowRate - 0.5 * u_step), (endTime, inflowRate - 0.5 * u_step)])
                        print("JOIN inflowrat: " + str(inflowRate) + ", interval: " + str(
                            intervalCounter) + ", arc: " + str(e) + ", l" + str(v) + " = " + str(
                            thinFlow.NTFNodeLabelDict[v]) + ", l" + str(w) + " = " + str(
                            thinFlow.NTFNodeLabelDict[w]) + ", oldl" + str(v) + " = " + str(
                            oldNodeLabel[v]) + ", oldl" + str(w) + " = " + str(oldNodeLabel[w]))
                    if abs(thinFlow.NTFNodeLabelDict[v] - thinFlow.NTFNodeLabelDict[w]) > TOL and abs(
                            oldNodeLabel[v] - oldNodeLabel[w]) < TOL:
                        clusterSplit.append([(oldInterval[intervalCounter][0], inflowRate - u_step), (
                            timeLimit * 1000 if oldInterval[intervalCounter][1] == float('inf') else
                            oldInterval[intervalCounter][1],
                            inflowRate - u_step)])  # Insert Split line to the old inflow rate for nice pictures
                        print("SPLIT inflowrat: " + str(inflowRate) + ", interval: " + str(
                            intervalCounter) + ", arc: " + str(e) + ", l" + str(v) + " = " + str(
                            thinFlow.NTFNodeLabelDict[v]) + ", l" + str(w) + " = " + str(
                            thinFlow.NTFNodeLabelDict[w]) + ", oldl" + str(v) + " = " + str(
                            oldNodeLabel[v]) + ", oldl" + str(w) + " = " + str(oldNodeLabel[w]))
    ...
    for k in list(
            oldInterval.keys()):  # if there are less intervals then before the non existing intervals must be deleted
