...
class NashFlow:
    ...
    def __init__(self, nodes, edges, transitTimeDict, capacityDict, inflowRate):
        ...
        for v, w in self.network.edges_iter():
            self.network[v][w]['transitTime'] = transitTimeDict[(v,w)]
            self.network[v][w]['capacity'] = capacityDict[(v,w)]
    ...
    def compute_flowInterval(self):
        ...
        interval = FlowInterval(self.network, resettingEdges=resettingEdges, lowerBoundTime=lowerBoundTime, inflowRate=self.inflowRate, minCapacity=self.minCapacity, counter=self.counter)
